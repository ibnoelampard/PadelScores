export const LANGUAGE_STORAGE_KEY = "padelscore-language-v1";

const messages = {
  id: {
    "language.switch": "Ganti bahasa ke English",
    "empty.title": "Match kamu",
    "empty.subtitle": "Atur rotasi padel dengan mudah",
    "empty.heading": "Belum ada match",
    "empty.description": "Buat sesi baru untuk mulai mix pemain dan menyusun jadwal pertandingan.",
    "empty.create": "＋  Buat Match",
    "setup.title": "Buat Match",
    "setup.subtitle": "Langkah 1 dari 3 · Atur sesi",
    "setup.players": "Jumlah pemain",
    "setup.courts": "Jumlah lapangan",
    "setup.duration": "Durasi sesi (jam)",
    "setup.examplePlayers": "Contoh: 12",
    "setup.exampleCourts": "Contoh: 2",
    "setup.exampleDuration": "Contoh: 2",
    "setup.rotation": "Rotasi otomatis setiap 10 menit",
    "setup.create": "Buat Match",
    "common.cancel": "Batal",
    "validation.setup": "Isi jumlah pemain minimal 4, lapangan minimal 1, dan durasi lebih dari 0.",
    "players.title": "Nama Pemain",
    "players.subtitle": "{count} pemain · Langkah 2 dari 3",
    "players.placeholder": "Nama pemain {number}",
    "players.create": "Buat Jadwal Pertandingan",
    "players.heading": "Isi semua nama pemain",
    "players.count": "{count} nama",
    "validation.namesRequired": "Semua nama pemain harus diisi.",
    "validation.namesDuplicate": "Nama pemain tidak boleh duplikat.",
    "schedule.title": "Jadwal Pertandingan",
    "schedule.subtitle": "{hours} jam",
    "schedule.chooseCourt": "Pilih lapangan",
    "schedule.matches": "{count} pertandingan",
    "schedule.bye": "Istirahat slot ini: {names}",
    "schedule.saved": "Tersimpan otomatis",
    "schedule.reset": "Reset sesi",
    "schedule.resetConfirm": "Hapus sesi dan semua jadwal?",
    "session.actions": "Aksi sesi",
    "session.addPlayer": "Tambah pemain",
    "session.removePlayer": "Pemain pulang",
    "session.addCourt": "Tambah lapangan",
    "session.removeCourt": "Tutup lapangan",
    "session.extendDuration": "Tambah durasi",
    "session.language": "Bahasa",
    "playerAdd.title": "Tambah pemain",
    "playerAdd.description": "Match yang sedang bermain atau sudah selesai tetap tidak berubah.",
    "playerAdd.name": "Nama pemain baru",
    "playerAdd.submit": "Tambah pemain",
    "durationAdd.title": "Tambah durasi",
    "durationAdd.description": "Slot baru akan dilanjutkan setelah jadwal terakhir.",
    "durationAdd.hours": "Tambahan durasi (jam)",
    "durationAdd.submit": "Tambah durasi",
    "validation.playerRequired": "Nama pemain wajib diisi.",
    "validation.playerDuplicate": "Nama pemain sudah ada.",
    "validation.durationInvalid": "Durasi tambahan harus lebih dari 0 jam.",
    "playerAdd.success": "{name} ditambahkan dan jadwal pending sudah di-mix ulang.",
    "playerAdd.noPending": "{name} ditambahkan. Tidak ada jadwal pending; pemain akan diprioritaskan di slot tambahan berikutnya.",
    "playerRemove.title": "Pemain pulang",
    "playerRemove.description": "Pemain tidak akan ikut slot berikutnya. Match yang sedang dimainkan harus diselesaikan atau diganti dahulu.",
    "playerRemove.label": "Pilih pemain",
    "playerRemove.submit": "Keluarkan pemain",
    "playerRemove.success": "Pemain dikeluarkan dari rotasi berikutnya.",
    "playerRemove.error": "Pemain yang sedang bermain belum dapat dikeluarkan.",
    "courtRemove.title": "Tutup lapangan",
    "courtRemove.action": "Tutup lapangan",
    "courtRemove.description": "Lapangan tidak dipakai untuk slot berikutnya. Match yang sedang dimainkan harus diselesaikan dahulu.",
    "courtRemove.confirm": "Tutup {name}? Lapangan ini tidak akan dipakai untuk jadwal berikutnya.",
    "courtRemove.label": "Pilih lapangan",
    "courtRemove.submit": "Tutup lapangan",
    "courtRemove.success": "Lapangan dikeluarkan dari rotasi berikutnya.",
    "courtRemove.error": "Lapangan yang sedang dipakai belum dapat ditutup.",
    "durationAdd.success": "{count} slot baru ditambahkan.",
    "durationAdd.error": "Durasi belum dapat ditambahkan.",
    "courtAdd.success": "Lapangan {number} ditambahkan. Jadwal mulai slot berikutnya sudah diperbarui.",
    "courtAdd.error": "Lapangan belum dapat ditambahkan.",
    "session.chooseTab": "Pilih tampilan sesi",
    "session.schedule": "Jadwal",
    "session.leaderboard": "Klasemen",
    "leaderboard.title": "Klasemen Pemain",
    "leaderboard.subtitle": "Hasil pertandingan yang selesai",
    "leaderboard.heading": "Statistik pemain",
    "leaderboard.matchesFinished": "{count} pertandingan selesai",
    "leaderboard.player": "Pemain",
    "leaderboard.matches": "Main",
    "leaderboard.wins": "Menang",
    "leaderboard.draws": "Seri",
    "leaderboard.losses": "Kalah",
    "court.label": "Lapangan {number}",
    "status.finished": "✓ Selesai",
    "status.playing": "● Sedang bermain",
    "score.label": "SKOR",
    "score.left": "Skor tim kiri",
    "score.right": "Skor tim kanan",
    "match.edit": "Edit",
    "match.start": "Mulai",
    "match.finish": "Selesai",
    "match.replace": "Ubah pemain",
    "replace.hint": "Ketuk nama untuk mengganti",
    "replace.title": "Ganti {name}",
    "replace.choose": "Pilih pemain pengganti yang tersedia",
    "replace.unselected": "Belum ada pengganti dipilih",
    "replace.out": "Pemain yang diganti",
    "replace.in": "Pemain pengganti",
    "replace.notice": "Match setelahnya akan di-mix ulang. Match bermain dan selesai tidak berubah.",
    "replace.save": "Ganti pemain",
    "replace.success": "Pemain diganti dan jadwal berikutnya sudah di-mix ulang.",
    "replace.none": "Tidak ada pemain istirahat yang tersedia.",
    "replace.error": "Penggantian tidak dapat dilakukan. Coba buka ulang jadwal."
  },
  en: {
    "language.switch": "Switch language to Indonesian",
    "empty.title": "Your matches",
    "empty.subtitle": "Set up padel rotations with ease",
    "empty.heading": "No matches yet",
    "empty.description": "Create a new session to mix players and build the match schedule.",
    "empty.create": "＋  Create Match",
    "setup.title": "Create Match",
    "setup.subtitle": "Step 1 of 3 · Set up your session",
    "setup.players": "Number of players",
    "setup.courts": "Number of courts",
    "setup.duration": "Session duration (hours)",
    "setup.examplePlayers": "Example: 12",
    "setup.exampleCourts": "Example: 2",
    "setup.exampleDuration": "Example: 2",
    "setup.rotation": "Automatic rotation every 10 minutes",
    "setup.create": "Create Match",
    "common.cancel": "Cancel",
    "validation.setup": "Enter at least 4 players, 1 court, and a duration greater than 0.",
    "players.title": "Player Names",
    "players.subtitle": "{count} players · Step 2 of 3",
    "players.placeholder": "Player name {number}",
    "players.create": "Create Match Schedule",
    "players.heading": "Enter every player's name",
    "players.count": "{count} names",
    "validation.namesRequired": "Every player name is required.",
    "validation.namesDuplicate": "Player names must be unique.",
    "schedule.title": "Match Schedule",
    "schedule.subtitle": "{hours} hours",
    "schedule.chooseCourt": "Choose a court",
    "schedule.matches": "{count} matches",
    "schedule.bye": "Resting this slot: {names}",
    "schedule.saved": "Saved automatically",
    "schedule.reset": "Reset session",
    "schedule.resetConfirm": "Delete this session and all its matches?",
    "session.actions": "Session actions",
    "session.addPlayer": "Add player",
    "session.removePlayer": "Player leaves",
    "session.addCourt": "Add court",
    "session.removeCourt": "Close court",
    "session.extendDuration": "Extend duration",
    "session.language": "Language",
    "playerAdd.title": "Add player",
    "playerAdd.description": "Playing and completed matches stay unchanged.",
    "playerAdd.name": "New player name",
    "playerAdd.submit": "Add player",
    "durationAdd.title": "Extend duration",
    "durationAdd.description": "New slots continue after the last scheduled slot.",
    "durationAdd.hours": "Additional duration (hours)",
    "durationAdd.submit": "Extend duration",
    "validation.playerRequired": "Player name is required.",
    "validation.playerDuplicate": "That player name already exists.",
    "validation.durationInvalid": "Additional duration must be greater than 0 hours.",
    "playerAdd.success": "{name} added and pending matches remixed.",
    "playerAdd.noPending": "{name} added. There are no pending matches; they will be prioritized in the next added slots.",
    "playerRemove.title": "Player leaves",
    "playerRemove.description": "The player will not join future slots. Finish or replace their active match first.",
    "playerRemove.label": "Choose player",
    "playerRemove.submit": "Remove player",
    "playerRemove.success": "Player removed from future rotations.",
    "playerRemove.error": "A player who is currently playing cannot be removed.",
    "courtRemove.title": "Close court",
    "courtRemove.action": "Close court",
    "courtRemove.description": "The court will not be used for future slots. Finish its active match first.",
    "courtRemove.confirm": "Close {name}? This court will not be used for future matches.",
    "courtRemove.label": "Choose court",
    "courtRemove.submit": "Close court",
    "courtRemove.success": "Court removed from future rotations.",
    "courtRemove.error": "A court currently in use cannot be closed.",
    "durationAdd.success": "{count} new slots added.",
    "durationAdd.error": "The duration could not be extended.",
    "courtAdd.success": "Court {number} added. Upcoming slots have been updated.",
    "courtAdd.error": "The court could not be added.",
    "session.chooseTab": "Choose a session view",
    "session.schedule": "Schedule",
    "session.leaderboard": "Leaderboard",
    "leaderboard.title": "Player Leaderboard",
    "leaderboard.subtitle": "Completed match results",
    "leaderboard.heading": "Player statistics",
    "leaderboard.matchesFinished": "{count} matches finished",
    "leaderboard.player": "Player",
    "leaderboard.matches": "Played",
    "leaderboard.wins": "Wins",
    "leaderboard.draws": "Draws",
    "leaderboard.losses": "Losses",
    "court.label": "Court {number}",
    "status.finished": "✓ Finished",
    "status.playing": "● Playing now",
    "score.label": "SCORE",
    "score.left": "Left team score",
    "score.right": "Right team score",
    "match.edit": "Edit",
    "match.start": "Start",
    "match.finish": "Finish",
    "match.replace": "Edit players",
    "replace.hint": "Tap a name to replace",
    "replace.title": "Replace {name}",
    "replace.choose": "Choose an available replacement player",
    "replace.unselected": "No replacement selected",
    "replace.out": "Player to replace",
    "replace.in": "Replacement player",
    "replace.notice": "Later matches will be remixed. Playing and completed matches stay unchanged.",
    "replace.save": "Replace player",
    "replace.success": "Player replaced and later matches remixed.",
    "replace.none": "No resting player is available.",
    "replace.error": "The replacement could not be made. Please reopen the schedule."
  }
};

export function resolveLanguage(browserLanguage, storedLanguage) {
  if (storedLanguage === "id" || storedLanguage === "en") return storedLanguage;
  return String(browserLanguage || "").toLowerCase().startsWith("id") ? "id" : "en";
}

export function createI18n({ storage = globalThis.localStorage, browserLanguage = globalThis.navigator?.language, root = globalThis.document?.documentElement } = {}) {
  let language = resolveLanguage(browserLanguage, storage?.getItem(LANGUAGE_STORAGE_KEY));
  const applyDocumentLanguage = () => root?.setAttribute("lang", language);
  applyDocumentLanguage();

  return {
    getLanguage: () => language,
    setLanguage(nextLanguage) {
      language = resolveLanguage(browserLanguage, nextLanguage);
      storage?.setItem(LANGUAGE_STORAGE_KEY, language);
      applyDocumentLanguage();
    },
    t(key, variables = {}) {
      const template = messages[language][key] || messages.en[key] || key;
      return template.replace(/\{(\w+)\}/g, (_, name) => String(variables[name] ?? ""));
    }
  };
}
