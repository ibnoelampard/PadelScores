...
export function generateSchedule({ players, courts, durationHours, previousSchedule = [] }) {
  ...
  for (let slotIndex = 0; slotIndex < slots; slotIndex += 1) {
    ...
    matches.forEach(([leftTeam, rightTeam], courtIndex) => {
      ...
      schedule.push({ id: `s${slotIndex + 1}-${courts[courtIndex].id}`, slotIndex, startMinute: slotIndex * 10, courtId: courts[courtIndex].id, teamA, teamB, scoreA: "", scoreB: "", started: false, finished: false, bye });
    });
